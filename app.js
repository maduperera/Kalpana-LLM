...
async function initKalpanaApp() {
  ...
  function updateActiveKpBanner() {
    ...
    if (subEl) {
      subEl.textContent = `${activePack.documents.length} Docs • ${(activePack.totalTokens || 0).toLocaleString()} Tokens • ${kernel.getMemoryUsageMB()} MB RIF Tensor (FP16) • 0 MB KV Cache`;
    }
  }
